function sBarChart (canvas, data, options) {
  ...
  this.data = data // 存放图标数据
  this.dataLength = this.data.length // 图标数据的长度
  ...
  this.xLength = 0 // x 轴坐标点之间的真是长度
  this.yFictitious = 0 // y轴坐标点之间显示的间距
  this.yRatio = 0 // y轴坐标真实长度和坐标间距的比例
  this.bgColor = '#fff' // 默认背景颜色
  this.fillColor = '#1e9fff' // 默认填充颜色
  this.axisColor = '#666666' // 坐标轴的颜色
  this.contentColor = '#eeeeee' // 内容横线颜色
  this.titleColor = '#000000' // 图标标题颜色
  this.title = ''
...
sBarChart.prototype = {
  init: function (options) {
    if (options) {
      ...
      this.titleColor = options.contentColor || '#000000'
  ...
  watchHover: function () {
    ...
    self.canvas.addEventListener('mouseover', function (e) {
      ...
      for (var i = 0; i < self.data.length; i++) {
        if (e.offsetX > self.data[i].left &&
            e.offsetX < self.data[i].right &&
            e.offsetY > self.data[i].top &&
            e.offsetY < self.data[i].bottom
        ) {
          self.currentIndex = i
        }
      }
  ...
  drawHover: function () {
    if (this.currentIndex !== -1) {
      if (this.onceMove === -1) {
        ...
        this.canvas.style.color = 'pointer'
      }
    } else {
      if (this.onceMove !== -1) {
        this.onceMove = -1
